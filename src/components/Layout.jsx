...
const Layout = ({ children }) => {
  ...
  const mainAvatar = "https://randomuser.me/api/portraits/men/20.jpg";
  const navs = [
    "Home",
    "Stages & Checklist",
    "Upload Docs",
    ...
    <div className="h-screen flex flex-col lg:flex-row">
      ...
      {isMenuOpen && (
        <div className="xl:hidden fixed top-14 left-0 w-full h-fit  bg-(--navbar) text-(--navbar-text) z-40 p-4 pb-20">
          <div className="space-y-3">
            {navs.map((v, index) => (
              <div key={index} className={"hover:bg-[#eff5f9] cursor-pointer px-4 py-2 rounded-sm " +  (index == 0 ? "text-[#2FBCFE] bg-[#FFFFFF1A]": "text-(--navbar-text)")}>
                {v}
              </div>
            ))}
          </div>
      ...
      <div className="hidden border xl:flex xl:w-1/6 xl:text-[100%] text-[80%] bg-(--navbar) text-(--navbar-text) h-full">
        <div className="w-full p-6 flex flex-col space-y-3 mt-auto h-[95%]">
          {navs.map((v, index) => (
            <div key={index} className={"hover:bg-[#eff5f9] cursor-pointer px-4 py-2 rounded-sm " +  (index == 0 ? "text-[#2FBCFE] bg-[#FFFFFF1A]": "text-(--navbar-text) hover:text-(--navbar)")}>
              {v}
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 lg:ml-0 scroll">
        ...
        {/* Main Content Area */}
        <main className={`container  ${isMobile ? 'mt-16 ml-auto mr-auto' : ''}`}>
          {children}
        </main>
      </div>
    </div>
